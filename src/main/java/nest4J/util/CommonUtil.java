...
import nest4J.util.coor.ClipperCoor;
import nest4J.util.coor.NestCoor;
...
public class CommonUtil {
    ...
    public static NestPath Path2NestPath (Path path){
        ...
        for(int  i = 0; i<path.size() ; i ++){
            ...
            NestCoor coor = nest4J.util.CommonUtil.toNestCoor(lp.getX(), lp.getY());
    ...
    public static Path NestPath2Path (NestPath nestPath ){
        ...
        for(Segment s : nestPath.getSegments()){
            ClipperCoor coor = nest4J.util.CommonUtil.toClipperCoor(s.getX(), s.getY());
    ...
    public static ClipperCoor toClipperCoor(double x , double y ){
        return new ClipperCoor((long)(x* nest4J.util.Config.CLIIPER_SCALE) , (long) (y * nest4J.util.Config.CLIIPER_SCALE));
    }
    ...
    public static NestCoor toNestCoor(long x , long y ){
        return new NestCoor(((double)x/ nest4J.util.Config.CLIIPER_SCALE) , ((double)y/ nest4J.util.Config.CLIIPER_SCALE));
    }
    ...
    public static List<NestPath> polygonOffset(NestPath polygon , double offset){
        ...
        for(Segment s : polygon.getSegments()){
            ClipperCoor cc = nest4J.util.CommonUtil.toClipperCoor(s.getX(), s.getY());
            p.add(new Point.LongPoint( cc.getX() ,cc.getY()));
        }

        int miterLimit = 2;
        ClipperOffset co = new ClipperOffset(miterLimit , polygon.config.CURVE_TOLERANCE * nest4J.util.Config.CLIIPER_SCALE);
        ...
        for(int i = 0 ; i <newpaths.size() ; i ++){
            result.add(nest4J.util.CommonUtil.clipperToNestPath(newpaths.get(i)));
        }
    ...
    public static List<NestPath> BuildTree(List<NestPath> parts ,double curve_tolerance){
        ...
        nest4J.util.CommonUtil.toTree(polygons, 0);
        return polygons;
    }
}
