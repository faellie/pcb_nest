...
import nest4J.util.GeometryUtil;
...
public class Placementworker {
    ...
    public Result placePaths(List<NestPath> paths){
        ...
        for(int i = 0 ; i<paths.size() ; i++){
            NestPath r = nest4J.util.GeometryUtil.rotatePolygon2Polygon(paths.get(i), paths.get(i).getRotation());
        ...
        double fitness = 0 ;
        double binarea = Math.abs(nest4J.util.GeometryUtil.polygonArea(this.binPolygon));
        String key = null;
        ...
        while(paths.size() > 0 ){
            ...
            for(int i = 0; i <paths.size() ; i ++){
                ...
                for(int j = 0 ; j < finalNfpf.size() ; j ++){
                    nf = finalNfpf.get(j);
                    if(Math.abs( nest4J.util.GeometryUtil.polygonArea(nf)) < 2 ){
                        continue;
                    }
                    for(int k = 0 ; k< nf.size() ; k ++){
                        ...
                        Bound rectBounds = nest4J.util.GeometryUtil.getPolygonBounds(allpoints);
