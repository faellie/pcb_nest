...
import pcbnest.utils.dxf2svg.model.Orphan;
...
public class PolylineUtils {
    ...
    public static ArrayList<String> convertFileToSvgStr(String aInFileName, Bound aInBound) {
        ...
        try {
            lInputStream = new FileInputStream(aInFileName);
        ...
        String lLayerId = CommonUtils.fileNameToLayer(aInFileName);
        DXFLayer layer = doc.getDXFLayer(lLayerId);
        ...
        addPlToArray(lOrederedPl, lines, lLayerId);
        aInBound.setMaxX(getBound(lOrederedPl).getMaxX());
        aInBound.setMaxY(getBound(lOrederedPl).getMaxY());
        return lines;
    }

    public static String  convertFileToSvgFile(String aInFileName, Bound aInBound) {
        ArrayList<String> lines = new ArrayList<String>();
        ...
        String lOutFile = aInFileName.replace(".dxf", ".svg");
        PrintWriter writer = null;
        try {
            writer = new PrintWriter(lOutFile, "UTF-8");
        } catch (FileNotFoundException e) {
            System.out.println("failed to open the file " + lOutFile + " : " + e);
            return null;
        } catch (UnsupportedEncodingException e) {
            System.out.println("failed to open the file " + lOutFile + " : " + e);
            return null ;
        }
        writer.println("<svg viewBox=\"0 0 " +  CommonUtils.vew_port_width+ " " + CommonUtils.vew_port_hight + "  \" xmlns=\"http://www.w3.org/2000/svg\">");
    ...
    private static void addPlToArray(List<DXFPolyline> aInPlList, ArrayList<String> aInLines, String aInLayerId) {
        boolean isFirst = true;
        aInLines.add("<path id=\"" + aInLayerId + "\" d = \"");
        ...
        //aInLines.add("z \" stroke=\"" + "green" +  " \" fill = \"none\" />");
        return;
    }
}
